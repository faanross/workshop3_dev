...
func (rl *windowsShellcode) DoShellcode(
	...
	if runtime.GOOS != "windows" {
		return models.LoadResult{Message: "Loader is Windows-only"}, fmt.Errorf("windowsReflectiveLoader called on non-Windows OS: %s", runtime.GOOS)
	}
	if len(dllBytes) == 0 {
		return models.LoadResult{Message: "No DLL bytes provided"}, errors.New("empty DLL bytes")
	}
	if exportName == "" {
		return models.LoadResult{Message: "Export name not specified"}, errors.New("export name required for DLL execution")
	}
	...
	if err := binary.Read(reader, binary.LittleEndian, &dosHeader); err != nil {
		return models.LoadResult{Message: "Failed to read DOS header"}, fmt.Errorf("read DOS header: %w", err)
	}
	if dosHeader.Magic != IMAGE_DOS_SIGNATURE {
		return models.LoadResult{Message: "Invalid DOS signature"}, errors.New("invalid DOS signature")
	}
	if _, err := reader.Seek(int64(dosHeader.Lfanew), 0); err != nil {
		return models.LoadResult{Message: "Failed to seek to NT Headers"}, fmt.Errorf("seek NT Headers: %w", err)
	}
	var peSignature uint32
	if err := binary.Read(reader, binary.LittleEndian, &peSignature); err != nil {
		return models.LoadResult{Message: "Failed to read PE signature"}, fmt.Errorf("read PE signature: %w", err)
	}
	if peSignature != IMAGE_NT_SIGNATURE {
		return models.LoadResult{Message: "Invalid PE signature"}, errors.New("invalid PE signature")
	}
	var fileHeader IMAGE_FILE_HEADER
	if err := binary.Read(reader, binary.LittleEndian, &fileHeader); err != nil {
		return models.LoadResult{Message: "Failed to read File Header"}, fmt.Errorf("read File Header: %w", err)
	}
	var optionalHeader IMAGE_OPTIONAL_HEADER64
	if err := binary.Read(reader, binary.LittleEndian, &optionalHeader); err != nil {
		return models.LoadResult{Message: "Failed to read Optional Header"}, fmt.Errorf("read Optional Header: %w", err)
	}
